...
public class HibernateUtil_CompositeQuery_rest_ifo {
	...
	public static Predicate get_aPredicate_For_AnyDB(CriteriaBuilder builder, Root<RestIfoVO> root, String columnName,
		...
		if ("restLoc".equals(columnName)) // 用於Integer
			...
			restMapVO.setMapID(Integer.valueOf(value));
			predicate = builder.equal(root.get("restMapVO"), restMapVO);
		}
...
//		Session session = HibernateUtil.getSessionFactory().openSession();
		...
		try {
			...
			criteriaQuery.orderBy(builder.asc(root.get("restLoc")));
