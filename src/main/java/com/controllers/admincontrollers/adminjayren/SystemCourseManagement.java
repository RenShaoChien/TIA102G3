...
//        return "redirect:/course/systemCourseList";
    ...
    @ResponseBody
    public Map<String, Set<String>> getSportEventsAndEquipment(@RequestParam String sportType) {
    ...
    public void dBGifReader(@RequestParam("picID") Integer picID, @RequestParam("source") String source, HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            switch (source) {
                case "systemCoursePic":
                    out.write(systemCoursePicService.selectSystemCoursePicByID(picID).getPic());
                    break;
                case "coachCoursePic":
                    out.write(ccService.selectCoachCoursePicByID(picID).getPic());
                    break;
                case "productPic":
