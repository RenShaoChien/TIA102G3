...
public class SystemCourseManagement {
    ...
    @RequestMapping(value = "/systemCourseList", method = {RequestMethod.GET, RequestMethod.POST})
    public String systemCourseList(String oper, String keyword, Integer pageNo, HttpServletRequest req) throws Exception {
...
//        List<SystemCourse> systemCourseList = systemCourseService.getSystemCoursesList(keyword, pageNo);
        List<SystemCourse> systemCourseList = systemCourseService.getSystemCoursesList(keyword, offset);
        session.setAttribute("systemCourses", systemCourseList);

        int totalRecords = systemCourseService.getSystemCourseCount(keyword).intValue();
        int pageCount = (int) Math.ceil((double) totalRecords / 5); // 計算總頁數
//        int pageCount = systemCourseService.getPageCount(keyword);
//        System.out.println("pageCount = " + pageCount);
    ...
    @GetMapping("/edit")
    public String editSystemCourse(ModelMap model, @RequestParam Integer systemCourseID) throws Exception {
        if (systemCourseID != null) {

            SystemCourse systemCourse = systemCourseService.getSystemCourseById(systemCourseID);
            List<SystemCoursePic> pics = systemCourseService.getSystemCoursePicsBySystemCourseId(systemCourseID);
...
//        return "redirect:/course/systemCourseList";
    ...
    @GetMapping("/getSportEventsAndEquipment")
    @ResponseBody
    public Map<String, Set<String>> getSportEventsAndEquipment(@RequestParam String sportType) throws Exception {
        Set<String> sportEvents = sportEventService.getSportEventsFromTypeSet(sportType);
        Set<String> sportEquipment = sportEventService.getSportEquipmentFromTypeSet(sportType);

        Map<String, Set<String>> responseData = new HashMap<>();
        responseData.put("sportEvents", sportEvents);
        responseData.put("sportEquipment", sportEquipment);
    ...
    public String delSystemCourse(ModelMap model, @RequestParam Integer systemCourseID, HttpSession session) throws Exception {
        ...
        if (systemCourseID != null) {
            // 刪除系統課程
            systemCourseService.deleteSystemCourse(systemCourseID);
            ...
            return "redirect:/course/systemCourseList?pageNo=" + pageNo + (keyword != null ? "&keyword=" + keyword : "");
        }
        return "error";
    }

    @GetMapping("/DBGifReader")
    public void dBGifReader(@RequestParam("picID") Integer picID, HttpServletRequest req, HttpServletResponse res)
            throws IOException {
        res.setContentType("image/*");
        ServletOutputStream out = res.getOutputStream();

        try {
            out.write(systemCoursePicService.selectSystemCoursePicByID(picID).getPic());
        } catch (Exception e) {
